...
export class MatchScene extends Phaser.Scene {
    ...
    createHUD() {
        ...
        // Smoke cooldown indicator - FIXED TO CAMERA
        this.smokeCDText = this.add.text(width - 80, height - 20, '💨 Ready', {
            font: '14px Arial',
            fill: '#00ff88'
        }).setOrigin(0.5).setDepth(100).setScrollFactor(0);
    ...
    createTouchControls() {
        ...
        // Joystick (left side) - FIXED TO CAMERA
        this.joystickBase = this.add.image(90, height - 120, 'joystick_base').setAlpha(0.6).setScrollFactor(0);
        this.joystickThumb = this.add.image(90, height - 120, 'joystick_thumb').setAlpha(0.8).setScrollFactor(0);
        
        // Action buttons (right side) - will be fixed in createActionButton
        this.smokeBtn = this.createActionButton(width - 150, height - 150, '💨', () => this.useSmoke());
        this.actionBtn = this.createActionButton(width - 70, height - 150, '🚩', () => this.startAction());
        this.sprintBtn = this.createActionButton(width - 70, height - 70, '🏃', null);
        
        this.touchControls.add([this.joystickBase, this.joystickThumb, this.smokeBtn, this.actionBtn, this.sprintBtn]);
        
        // Aim zone (right half) - FIXED TO CAMERA
        this.aimZone = this.add.rectangle(width * 0.75, height / 2, width / 2, height, 0x000000, 0);
    ...
    setupTouchInput() {
        ...
        this.input.on('pointerup', (pointer) => {
            ...
            // Reset firing when pointer is released
            if (this.isTouchDevice && pointer.x > width / 2) {
                this.isFiring = false;
            }
        });
        
        this.sprintBtn?.on('pointerdown', () => { this.isSprinting = true; });
        this.sprintBtn?.on('pointerup', () => { this.isSprinting = false; });
        this.sprintBtn?.on('pointerout', () => { this.isSprinting = false; });
    }
    ...
    update(time, delta) {
        // CRITICAL: If round ended, do minimal updates only
        if (this.roundEnded) {
            this.updateHUD();
            // Keep player/AI stopped
    ...
    hitPlayer(bullet) {
        console.log('[DEBUG] hitPlayer called - roundEnded:', this.roundEnded, 'player.active:', this.player?.active);
        
        // Guard: don't process if round already ended
        if (this.roundEnded || !this.player?.active) {
            console.log('[DEBUG] hitPlayer BLOCKED - roundEnded or player inactive');
            bullet?.destroy?.();
            return;
        }
        
        let damage = bullet?.damage ?? 20;
        console.log('[DEBUG] Processing damage:', damage, 'Current HP:', this.playerHP);
        bullet?.destroy?.();
        ...
        console.log('[DEBUG] HP after damage:', this.playerHP, 'Checking if <= 0...');
        if (this.playerHP <= 0 && !this.roundEnded) {
            console.log('[DEBUG] *** PLAYER HP <= 0! Calling playerDied() ***');
            this.playerDied();
        }
    ...
    hitAI(bullet) {
        ...
        if (this.aiHP <= 0 && !this.roundEnded) {
            this.aiDied();
        }
    ...
    playerDied() {
        ...
        // Add to kill feed
        this.addToKillFeed('AI', 'YOU', '🔫');
        
        // Record death stat
        SaveSystem.recordDeath();
        
        // AI callout
        this.showAICallout('Target neutralized!');
    ...
    startRound() {
        ...
        this.showMessage(`⚔️ Round ${this.roundNumber}`, 1500);
    }
    ...
    endRound(playerWon, reason) {
        console.log('[DEBUG] endRound called - playerWon:', playerWon, 'reason:', reason, 'wasRoundEnded:', this.roundEnded);
        
        if (this.roundEnded && reason !== 'eliminated') {
            console.log('[DEBUG] endRound skipped - already ended (unless from death)');
            return;
        }
        this.roundEnded = true;
        this.roundActive = false;
        
        console.log('[DEBUG] Round ended - Player won:', playerWon, 'Reason:', reason);
        ...
        // Check match end
        if (this.playerScore >= CONFIG.ROUNDS_TO_WIN || this.aiScore >= CONFIG.ROUNDS_TO_WIN) {
