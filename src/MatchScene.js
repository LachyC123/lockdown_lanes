...
export class MatchScene extends Phaser.Scene {
    ...
    init(data) {
        ...
        // Post-round transition safety (prevents getting stuck after death/end)
        this.roundEndAt = 0;
    ...
    update(time, delta) {
        // CRITICAL GUARD: If round ended OR roundEndProcessed, STOP ALL GAME LOGIC
        if (this.roundEnded || this.roundEndProcessed) {
            ...
            // Self-heal: if for any reason the scheduled transition never fires,
            // force the next round / results after a grace period.
            if (this.roundEndAt && !this._transitionFired) {
                const stuckFor = Date.now() - this.roundEndAt;
                if (stuckFor > 7000) {
        ...
        // SAFETY NET: if HP becomes invalid/NaN (e.g. from bad upgrade math),
        // force it to 0 so elimination can correctly end the round.
        if (!Number.isFinite(this.playerHP)) this.playerHP = 0;
        if (!Number.isFinite(this.aiHP)) this.aiHP = 0;

        // SAFETY NET: ensure elimination ALWAYS ends the round
        if (!this.roundEnded && this.roundActive) {
            if (this.playerHP <= 0 && this.player?.active) {
                this.playerDied();
                return;
            }
            if (this.aiHP <= 0 && this.ai?.active) {
                this.aiDied();
                return;
            }
        }
    ...
    hitPlayer(bullet) {
        ...
        // SAFETY: prevent NaN/invalid damage from breaking death checks
        if (!Number.isFinite(damage)) damage = 20;
    ...
    playerDied() {
        console.log('[DEATH] >>> playerDied() CALLED! roundEnded:', this.roundEnded, 'roundEndProcessed:', this.roundEndProcessed);
        
        // CRITICAL: Prevent ANY duplicate calls
        if (this.roundEnded || this.roundEndProcessed) {
            console.log('[DEATH] playerDied BLOCKED - already ended/processed');
            return;
        }
        
        // IMMEDIATELY SET ALL FLAGS - DO THIS FIRST!
        ...
        console.log('[DEATH] Player death - ALL FLAGS SET: roundEnded=true, roundActive=false, roundEndProcessed=true');
        ...
        this.matchStats.deaths = (this.matchStats.deaths ?? 0) + 1;
        ...
        // UPDATE SCORE DIRECTLY - Don't call endRound separately
        this.aiScore++;
        this.playerCredits += CONFIG.LOSE_CREDITS;
        console.log('[DEATH] Score updated: Player', this.playerScore, '- AI', this.aiScore);
        
        // Clean up bombs and smokes
        this.plantedBomb?.destroy?.();
        ...
        // Show round result
        this.showRoundResult(false, 'eliminated');
        this.playSound('lose');
        
        // Notify AI controller
        this.aiController?.onRoundEnd?.(true);
        
        // Check if match is over
        const matchOver = this.aiScore >= CONFIG.ROUNDS_TO_WIN;
        console.log('[DEATH] Match over check:', matchOver, '(aiScore:', this.aiScore, '/ ROUNDS_TO_WIN:', CONFIG.ROUNDS_TO_WIN, ')');

        // Mark time of round end (used for self-heal if timers stall)
        this.roundEndAt = Date.now();
        
        if (matchOver) {
            console.log('[DEATH] MATCH OVER - AI wins! Going to results in 2500ms...');
            this.schedulePostRoundTransition(2500, () => this.endMatch());
        } else {
            ...
            this.schedulePostRoundTransition(2500, () => {
                console.log('[DEATH] Starting next round...');
                this.startRound();
            });
        }
    }
    ...
    aiDied() {
        console.log('[DEATH] >>> aiDied() CALLED! roundEnded:', this.roundEnded, 'roundEndProcessed:', this.roundEndProcessed);
        
        // CRITICAL: Prevent ANY duplicate calls
        if (this.roundEnded || this.roundEndProcessed) {
            console.log('[DEATH] aiDied BLOCKED - already ended/processed');
            return;
        }
        ...
        console.log('[DEATH] AI death - ALL FLAGS SET: roundEnded=true, roundActive=false, roundEndProcessed=true');
        ...
        // Show round result
        this.showRoundResult(true, 'eliminated');
        this.playSound('win');
        
        // Notify AI controller
        this.aiController?.onRoundEnd?.(false);
        
        // Check if match is over
        const matchOver = this.playerScore >= CONFIG.ROUNDS_TO_WIN;
        console.log('[DEATH] Match over check:', matchOver, '(playerScore:', this.playerScore, '/ ROUNDS_TO_WIN:', CONFIG.ROUNDS_TO_WIN, ')');

        // Mark time of round end (used for self-heal if timers stall)
        this.roundEndAt = Date.now();
        
        if (matchOver) {
            console.log('[DEATH] MATCH OVER - Player wins! Going to results in 2500ms...');
            this.schedulePostRoundTransition(2500, () => this.endMatch());
        } else {
            // Swap sides for next round
            this.playerIsAttacker = !this.playerIsAttacker;
    ...
    startRound() {
        // Clear any pending post-round transitions/timeouts
        this.clearPostRoundTransition();
    ...
    // Schedules a post-round transition with a Phaser timer AND a real-time fallback.
    // This prevents rare cases where Phaser's clock stalls (tab throttling / timescale issues)
    // and the match gets stuck after an elimination.
    schedulePostRoundTransition(delayMs, fn) {
        ...
        const fireOnce = () => {
            if (this._transitionFired) return;
            this._transitionFired = true;
            this._transitionScheduled = false;
            try { fn?.(); } catch (e) { console.error(e); }
        };

        // Phaser clock (normal path)
        this.time?.delayedCall?.(delayMs, () => {
            if (!this.scene?.isActive?.()) return;
            fireOnce();
        }, [], this);

        // Real-time fallback (runs even if Phaser time is slowed/paused)
