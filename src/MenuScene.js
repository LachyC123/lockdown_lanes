...
export class MenuScene extends Phaser.Scene {
    ...
    create() {
        ...
        // Trophy display
        this.createTrophyDisplay(width / 2, 170);
        
        // Class indicator
        this.createClassIndicator(width / 2, 230);
        
        // Main buttons - left column
        this.createButton(width / 2 - 140, 290, '⚔️ RANKED', CONFIG.COLORS.PRIMARY, () => {
            this.showClassSelection('ranked');
        }, 130);
        
        this.createButton(width / 2 + 140, 290, '🎮 TRAINING', CONFIG.COLORS.SUCCESS, () => {
            this.showTrainingMenu();
        }, 130);
        
        this.createButton(width / 2 - 140, 355, '🌟 CLASSES', 0x9b59b6, () => {
            this.showClassSelection(null);
        }, 130);
        
        this.createButton(width / 2 + 140, 355, '🔓 UNLOCKS', 0x34495e, () => {
            this.showUnlocks();
        }, 130);
        
        // Bottom row - full width
        this.createButton(width / 2, 420, '📖 HOW TO PLAY', CONFIG.COLORS.WARNING, () => {
            this.showHowToPlay();
        });
        
        this.createButton(width / 2 - 140, 485, '📊 STATS', 0x2980b9, () => {
        ...
        this.createButton(width / 2 + 140, 485, '⚙️ SETTINGS', 0x7f8c8d, () => {
            this.cameras.main.fadeOut(300, 0, 0, 0);
            this.cameras.main.once('camerafadeoutcomplete', () => {
    ...
    createTitle() {
        ...
        const titleBottom = this.add.text(width / 2, 105, 'LANES', {
    ...
    createClassIndicator(x, y) {
        ...
        bg.fillRoundedRect(x - 100, y - 15, 200, 30, 8);
        bg.lineStyle(1, playerClass.color ?? 0x3498db);
        bg.strokeRoundedRect(x - 100, y - 15, 200, 30, 8);
        
        this.add.text(x, y, `${playerClass.icon ?? '⚔️'} ${playerClass.name ?? 'Assault'} Class`, {
